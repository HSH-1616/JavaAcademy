...
COMMENT ON COLUMN MEMBER.MEMBER_ID IS '회원 아이디 최소 8글자 이상 저장';
COMMENT ON COLUMN MEMBER.MEMBER_PW IS '회원 비밀번호 최소 8글자 이상 저장';
...
--PRIMARY KEY(P)/PK : 데이터(ROW) 구분하는 컬럼에 설정하는 제약조건 -> NOT NULL,UNIQUE제약 조건 설정
                    --일반적으로 한개 테이블에 한개 PK를 설정
                    --다수컬럼에 설정할 수도 있다(복합키)
--FOREGIN KEY(R) : 지정된 컬럼의 값을 다른 테이블의 지정된 컬럼에 있는 값만 지정하게 하는 제약조건
                --다른 컬럼에 지정된 컬럼은 중복이 있으면 안됨(UNIQUE제약조건이나, PK제약조건이 설정된 컬럼)
...
--1. 테이블 생성과 동시에 설정하기
--   1)컬럼레벨에서 설정
--      예) CREATE TABLE 테이블명( 컬럼명 자료형 제약조건,.....)
--   2)테이블레벨에서 설정
...
--제약조건이 설정되지 않으면 모든 컬럼에는 NULL값을 허용
...
--NOT NULL(MEMBER_NO)테이블에서 설정 불가능하다.

--UNIQUE제약조건
--컬럼이 유일한값을 유지해야할때 사용
SELECT * FROM BASIC_MEMBER;
INSERT INTO BASIC_MEMBER VALUES(1,'ADMIN',1234,'관리자',48);
INSERT INTO BASIC_MEMBER VALUES(2,'ADMIN',3333,'유저',21);
...
INSERT INTO NQ_MEMBER VALUES (2,'ADMIN',1234,'유저',44);

--NULL값에 대한 처리는 어떻게???
INSERT INTO NQ_MEMBER VALUES(3,NULL,'1234','유저2',23);
SELECT * FROM NQ_MEMBER;
...
INSERT INTO NQ_MEMBER2 VALUES (2,'ADMIN',1234,'관리자',44);
INSERT INTO NQ_MEMBER2 VALUES (3,'ADMIN',2222,'유저',22);
...
INSERT INTO NQ_MEMBER3 VALUES (1,NULL,1234,'관리자',44);
INSERT INTO NQ_MEMBER3 VALUES (2,'ADMIN',1234,'관리자',44);
INSERT INTO NQ_MEMBER3 VALUES (3,'ADMIN',2222,'유저',22);
...
MEMBER_NAME VARCHAR2(10),
MEMBER_AGE NUMBER,
UNIQUE(MEMBER_ID,MEMBER_NAME)-- 다수의 컬럼에 UNIQUE제약조건을 설정할때 사용
);
SELECT * FROM NQ_MEMBER4;
INSERT INTO NQ_MEMBER4 VALUES (1,'ADMIN',1234,'관리자',44);
INSERT INTO NQ_MEMBER4 VALUES (2,'ADMIN',1234,'관리자',44);
INSERT INTO NQ_MEMBER4 VALUES (3,'ADMIN',2222,'유저',22);
INSERT INTO NQ_MEMBER4 VALUES (4,'ADMIN1',2222,'유저',22);
INSERT INTO NQ_MEMBER4 VALUES (5,'ADMIN',22322,'유저',22);

--PRIMARY KEY 설정하기
--생성한 테이블의 컬럼 중 도메인이 중복값이 없고, NULL값을 허용하지 않을때 그 컬럼에 설정
--PK용 컬럼을 생성해서 활용 -> IDX, STUDENTNO, PRODUCTNO
--저장되는 데이터 중 하나를 선택해서 설정
--PK를 설정하면 자동으로 UNIQUE, NOT NULL제약 조건, INDEX가 부여됨
...
INSERT INTO PK_MEMBER VALUES (1,'USER01',1234,'유저1',22);
...
INSERT INTO PK_MEMBER1 VALUES (1,'USER01',1234,'유저1',22);
...
INSERT INTO PK_MEMBER2 VALUES (1,'USER01',2222,'유저2',22);
INSERT INTO PK_MEMBER2 VALUES (NULL,'USER02',3333,'유저3',33);
INSERT INTO PK_MEMBER2 VALUES (3,NULL,2222,'유저2',22);
SELECT * FROM PK_MEMBER2;

--구매 테이블, 장바구니 테이블 등에 복합키를 설정할 수 있다.
CREATE TABLE CART(
MEMBER_ID VARCHAR2(20),
PRODUCT_NO NUMBER,
BUY_DATE DATE,
...
--다른 테이블에 없는 데이터를 가져와 사용하는 것(참조)
--참조관계를 설정하면 부모(참조되는테이블)-자식(참조하는테이블) 관계가 설정이됨
--PK제약조건은 자식테이블에 설정
--PK제약조건을  설정하는 컬럼은 UNIQUE제약조건이나 PK제약조건이 설정되어있어야 한다.
CREATE TABLE BOARD(
...
--FK가 설정된 컬럼에 NULL? 저장된다. 저장하지 않으려면 NOT NULL(제약조건을 설정해야한다.)
INSERT INTO BOARD_COMMENT VALUES(5,'NULL들어가니?','D',SYSDATE,NULL);
SELECT * FROM BOARD_COMMENT;

--FK를 설정해서 테이블과 관계가 설정이 되면 참조되고는 있는 부모테이블의 ROW를 함부로 삭제할수 없다.
DELETE FROM BOARD WHERE BOARD_NO=2;
SELECT * FROM BOARD;

--FK 설정할때 삭제에 대한 옵션을 설정할 수 있다.
...
--제약설정시 이름설정하기
--기본방식으로 제약조건을 설정하면 SYSOOOOO으로 자동으로 설정함.
...
--INSERT문 활용하기
--1. 전체컬럼에 값을 대입하기
--INSERT INTO 테이블명 VALUES(컬럼에 대입할 값, 컬럼에 대입할 값, 커럼에 대입할 값..)*테이블에 선언된 컬럼수와 동일해야한다.
--2. 특정 컬럼을 골라서 값을 대입하기
--INSERT INTO 테이블명(특정컬럼, 특정컬럼....) VALUES(특정컬럼에 대입할 값, 특정 컬럼에 대입할 값....)
--* 지정된 컬럼의 수와 VALUES에 있는 수가 같아야함
--* 지정되지 않은 컬럼의 값은 NULL로 대입됨. 주의! 나머지 컬럼의 NOT NULL 제약조건이 있으면 안된다.
...
--UPDATE문 활용하기
--UPDATE 테이블명 SET 수정할 컬럼명=수정할값, 수정할 컬럼명=수정할값.....(WHERE 조건)
...
ROLLBACK;
UPDATE EMP_SALARY SET (DEPT_CODE,BONUS)=(SELECT DEPT_CODE,BONUS FROM EMPLOYEE WHERE EMP_NAME='심봉선')
WHERE EMP_NAME='방명수';
...
--DDL(ALTER,DROP)
--ALTRER ; 오라클에 정의되어있는 OBJECT를 수정할때 사용하는 명령어
...
--NOT NULL 제약조건은 이미 컬럼에 NULLABLE로 설정이 되어있기 때문에 ADD가 아닌 MODIFY변경으로 해줘야한다.
...
--제약조건명 변경하기
--ALTER TABLE 테이블명 RENAME CONSTRAINT 제약조건명 TO 제약조건명
...
--USER, TABLE, VIEW, SEQUENCE, INDEX, SYSNONYM,FUNCTION,PROCESIER,PACKAGE 등등
CREATE TABLE EMP_M1
AS SELECT * FROM EMPLOYEE;
CREATE TABLE EMP_M2
AS SELECT * FROM EMPLOYEE WHERE JOB_CODE='J5';

INSERT INTO EMP_M2 VALUES(999,'곽두원','561014`123456','ASD.COM','01012341234','15','J1','S1',90000,0.5,NULL
...
-- -> VIEW에서 값을 넣은것 이외에 컬럼에는 NULL값을 삽입함.->NOT NULL제약조건이 설정되면 안된다.
CREATE VIEW V_EMPTEST
AS SELECT EMP_ID, EMP_NO, EMP_NAME, EMAIL, PHONE, JOB_CODE, SAL_LEVEL FROM EMPLOYEE;
SELECT * FROM V_EMPTEST;

INSERT INTO V_EMPTEST VALUES('997','981234-1234123','홍길동','HONG@HONG.COM','12341234','J1','S1');

--JOIN을 사용한 VIEW는 INSERT가 불가능함.
--JOIN, UNION연결된 VIEW는 입력이 불가능하다.
INSERT INTO V_EMP VALUES('996','홍길동','981234-1234567','HONG@HONG.COM','12345','D5','J1','S1',
100,0.2,204,SYSDATE,NULL,'S','D0','되니?','L3');
...
--*OBJECT명칭은 중복이 불가능하다.
CREATE OR REPLACE VIEW V_EMP AS SELECT * FROM EMPLOYEE;
SELECT * FROM V_EMP;

--FORCE/NOFORCE : 실제 테이블이 존재하지 않아도 VIEW생성할수 있게 해주는 옵션
SELECT * FROM TT;
DROP VIEW V_TT;
...
--1.SEQUENCE번호를 발급하려면 시퀀스명.NEXTVAR을 선언한다.
CREATE SEQUENCE SEQ_BASIC;
SELECT SEQ_BASIC.NEXTVAL FROM DUAL;
--SEQUENCE데이터는 중복되지 않는 숫자를 발급해주기 때문데 TABLE의 PK컬럼의 값으로 많이 사용한다.
SELECT * FROM BOARD;
DESC BOARD;
...
--NEXTVALUE 숫자 : 최대값을 설정
--MINVALUE 숫자 : 최소값을 설정
--CYCLE/NOCYCLE : 번호를 순환할지 말지 결정하는 것 * MAXVALUE,MINVALUE가 설정되어 있어야 한다.
--CASHE : 미리번호를 생성하는 기능 * DEFAULT 20
SELECT * FROM USER_SEQUENCES;
CREATE SEQUENCE SEQ_01
START WITH 100;
SELECT SEQ_01.NEXTVAL FROM DUAL;

CREATE SEQUENCE SEQ_02 START WITH 100 INCREMENT BY 10;
...
--2. SEQUENCE에 값을 추가해서 PK값으로 쓸수있다.
SELECT 'P_'||LPAD(SEQ_05.NEXTVAL,4,'0') FROM DUAL;
SELECT TO_CHAR(SYSDATE,'YYYYMMDD')||'_'||SEQ_05.NEXTVAL FROM DUAL;

--오라클에서 제공하는 SELECT문
--계층형쿼리, 한 테이블에서 연관있는 ROW를 묶어서 출력할수 있게 해주는 쿼리문 -> ROW 순서를 연관있는 자룔로 연결해서 출력
--매니저와 연결해서 출력하기
SELECT LEVEL, EMP_ID, EMP_NAME, MANAGER_ID FROM EMPLOYEE
...
--자료형의 종류
--기본자료형 : 오라클에서 제공하는 TYPE (NUMBER, VARCHAR2, CHAR, DATE....)
--참조형자료형 : 테이블의 특정컬럼에 설정된 타입을 불러와 사용
--ROWTYPE : 테이블의 한개 ROW를 저장할수 있는 타입

--타입을 생성해서 활용
--TABLETYPE : 자바의 배열과 비슷한 타입 -> 인덱스번호가 인고, 한개타입만 저장이 가능
...
BEGIN
    ...
    IF V_SALARY>3000000
        THEN DBMS_OUTPUT.PUT_LINE('고액월급');
    END IF;    
END;
/
...
ELSE INTO LOWSAL VALUES(EMPID,SALARY);
END IF;
END;
/

--IF THEN ELSEIF 조건식 THEN ELSE END IF;
...
BEGIN
    LOOP
        ...
        INSERT INTO BOARD VALUES(SEQ_BASIC.NEXTVALM,'제목'||RNDNUM,'CONTENT'||RNDNUM,'작성자'||RNDNUM,SYSDATE);
        NUM:=NUM+1;
        IF NUM>100
...
BEGIN
    FOR EMP IN(SELECT * FROM EMPLOYEE) LOOP
        INSERT INTO EMP_DEL VALUES(EMP.EMP_ID, EMP.EMP_NAME, EMP.EMP_NO, EMP.EMAIL, EMP.PHONE,
        EMP.DEPT_CODE, EMP.JOB_CODE, EMP.SAL_LEVEL, EMP.SLALARY, EMP.BONUS, EMP.MANAGER_ID, 
        EMP.HIRE_DATE, EMP.ENT_DATE, EMP.ENT_YN,);
     END LOOP;
     COMMIT;
END;
/
...
SELECT MYFUNC('유병승ASD') FROM DUAL;
SELECT MYFUNC(EMAIL) FROM EMPLOYEE;

--매개변수로 EMP_ID를 받아서 연봉을 계산해주는 함수를 만들기
CREATE OR REPALCE FUNCTION SAL_YEAR(V_EMPID EMPLOYEE.EMP_ID%TYPE)
RETURN NUMBER
IS
    V_RESULT NUMBER;
BEGIN
...
FOR EACH ROW
BEGIN
    --트리거 실행시 새로입력한걸, 이전값에 대해 출력할수 있다.
    --INSERT : 새로운값(O), 이전값(X)
    --UPDATE : 새로운값(O), 이전값(O)
    --DELETE : 새로운값(O), 이전값(O)
    --새로운값 : :NEW.컬럼명
    --이전값: :OLD.컬럼명
    DBMS_OUTPUT.PUT_LINE('신입사원 '||:NEW.EMP_NAME||' 입사하였습니다.');    
END;
/
INSERT INTO EMPLOYEE VALUES('992','최D','960105-1234567','CHOI@CHOIK.COM','01012341234','D3','J1',
