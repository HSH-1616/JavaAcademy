...
//		
		...
		// student에 저장된 데이터 중 20살 이상인 남학생을
		// 나이 오름차순으로 정렬
		// menList에 저장하기
		...
		System.out.println("남학생만 출력하기");
		menList.forEach((s) -> System.out.println(s));

		menList = student.stream().filter((s) -> s.getGender() == Gender.M).filter((s) -> s.getAge() >= 50)
				.sorted((s, s1) -> s.getAge() - s1.getAge()).limit(5).collect(Collectors.toList());

		System.out.println("50세이상인 남자 출력하기");
		menList.forEach((s) -> System.out.println(s));
...
//		java.util.function.Predicate
		...
		// womenList에 저장 출력
		List<Student> womenList = student.stream().filter((s) -> s.getAge() >= 30)
				.filter((s) -> s.getGender() == Gender.F).collect(Collectors.toList());
		System.out.println("30이상 여학생 조회하기");
		...
		// 학생 키 평균 구하기
		avgAge = student.stream().collect(Collectors.averagingDouble((s) -> s.getHeight()));
		// .averagingDouble(Student::getHeight));
		System.out.println("학생의 평균키 : " + avgAge);

		// 원하는 값만 출력하기
		// 여학생 중 나이가 20대인 학생이름만 출력하기
		List<String> names = student.stream()
				.filter((s) -> s.getGender() == Gender.F && s.getAge() >= 20 && s.getAge() < 30).map((s) -> s.getName())
				// Map.of("n"+s.getStdNo(),s.getName()))
