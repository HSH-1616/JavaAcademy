...
public class NoticeInsertEndServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//3 : 업로드파일의 최대크기 설정 -> int (byte(1024)*1024(MB)*1024*(GB)*1024(TB))
		//4 : 인코딩방식 -> String(utf-8)
		//5 : rename규칙설정(클래스) -> 기본제공클래스(DefaultFileRenamePolicy)
		
		//String으로 저장할 위치(절대경로) 가져오기
		//ServletContext객체를 이용해서 웹애플리케이션의 절대 경로를 가져올 수 있다.
		...
		//클라이언트가 보낸 데이터는 생성된 MultipartRequest객체를 이용해서 가져온다 -> getParameter("name")
