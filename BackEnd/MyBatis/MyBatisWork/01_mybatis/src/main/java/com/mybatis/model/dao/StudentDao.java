...
public class StudentDao {
	...
	public List<Student> selectStudentPage(SqlSession session, int cPage, int numPerpage) {
		...
		// RowBounds클래스 이용, selectList호출시 세번째 매개변수에 RowBounds클래스를 생성해서 전달해주면 된다.
		// 매개변수있는 생성자를 이용
		// 1. 매개변수 : offset -> 시작row번호 (cPage-1) * numPerpage
		// 2. 매개변수 : 범위 -> numPerpage
		// new RowBounds((cPage-1)*numPerpage,numPerpage);
		RowBounds rb=new RowBounds((cPage-1)*numPerpage,numPerpage);
		List<Student> result = session.selectList("student.selectStudentPage",null,rb);
		return result;
	}
